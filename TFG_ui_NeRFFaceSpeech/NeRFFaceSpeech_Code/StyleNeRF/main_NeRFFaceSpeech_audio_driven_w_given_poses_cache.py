...
@click.option('--character_name', default=None, type=str, help='Character name for caching PTI and 3DMM results')
@click.option('--cache_dir', default=None, type=str, help='Custom cache directory for PTI and 3DMM results. If specified, this takes precedence over character_name')
def generate_images(
    ctx: click.Context,
    network_pkl: str,
    ...
    # 确定缓存目录的优先级：cache_dir > character_name > outdir
    if cache_dir is not None:
        # 使用用户指定的缓存目录
        character_dir = cache_dir
        os.makedirs(character_dir, exist_ok=True)
        print(f'Using custom cache directory: {character_dir}')
    elif character_name is not None:
        # 使用基于角色名的默认缓存目录
        character_dir = f"character/{character_name}"
        os.makedirs(character_dir, exist_ok=True)
        print(f'Using character-based cache directory: {character_dir}')
    else:
        # 使用输出目录作为缓存目录
        character_dir = outdir
        print(f'Using output directory as cache directory: {character_dir}')
    ...
    if not os.path.isfile(f"{outdir}/output_NeRFFaceSpeech.mp4"):
        ...
        with torch.no_grad():
            ...
            PIL.Image.fromarray(img[0], 'RGB').save(f'{character_dir}/output_from_w_pivot_w_G_PTI.png')
            #exit(0)
            ...
            PIL.Image.fromarray(proc_img(i_frame_224[:1])[0], 'RGB').save(f'{character_dir}/img_tesnor_224_resize_prc.png')
            ...
            PIL.Image.fromarray(img_tesnor_224_prc[0], 'RGB').save(f'{character_dir}/img_tesnor_224_resize_prc.png')
            
            from PIL import Image
            im = Image.open(f'{character_dir}/img_tesnor_224_resize_prc.png').convert('RGB')
        ...
        ## 3D fitting
        fitted_coeffs = fit_3dmm(character_dir)
        
        with torch.no_grad():
            _, _, proj_2d_vertex, _ = render_3dmm(Deep3Dmodel, img_tesnor_224_prc, fitted_coeffs, character_dir, save_option=True, prefix="fitted_img", get_lm=True)
