...
# 从 gradio_app/shared/utils/ 到项目根目录需要向上4级
...
def main():
    ...
    try:
        ...
        def ensure_json_serializable(obj):
            ...
            if isinstance(obj, bytes):
                try:
                    ...
                    # 如果UTF-8解码失败，使用errors='replace'处理非UTF-8字节
        ...
        try:
            ...
            # 如果错误消息本身有编码问题，使用repr
            try:
                error_message = repr(e)
            except:
                error_message = "发生未知错误（无法获取错误详情）"
        ...
        except (UnicodeEncodeError, TypeError) as json_error:
            # 如果JSON序列化失败，尝试使用ASCII编码
            error_result["error"]["message"] = error_message.encode('ascii', errors='replace').decode('ascii')
