...
def generate_video(
    ...
    audio_path = Path(audio_path).resolve()  # 转换为绝对路径
    test_img = Path(test_img).resolve()  # 转换为绝对路径
    network_path = network_path.resolve()  # 转换为绝对路径
    
    add_log(f"[NeRF] 输出目录（绝对路径）: {output_path}", "info")
    add_log(f"[NeRF] 音频文件（绝对路径）: {audio_path}", "info")
    add_log(f"[NeRF] 测试图片（绝对路径）: {test_img}", "info")
    add_log(f"[NeRF] 模型文件（绝对路径）: {network_path}", "info")
    ...
    # 使用系统默认的环境变量，不强制设置
    # 如果环境变量已设置，则使用；否则使用系统默认值
    if "TORCH_HOME" in env:
        add_log(f"[NeRF] 使用 TORCH_HOME: {env['TORCH_HOME']}", "info")
    else:
        add_log(f"[NeRF] 使用系统默认的 TORCH_HOME", "info")
    
    if "HF_ENDPOINT" in env:
        add_log(f"[NeRF] 使用 HF_ENDPOINT: {env['HF_ENDPOINT']}", "info")
    else:
        add_log(f"[NeRF] 使用系统默认的 HF_ENDPOINT", "info")
    
    if "HF_HOME" in env:
        add_log(f"[NeRF] 使用 HF_HOME: {env['HF_HOME']}", "info")
    else:
        add_log(f"[NeRF] 使用系统默认的 HF_HOME", "info")
    
    # 设置模型缓存目录，避免重复下载
    # face_alignment 和 torch.hub 通常使用 TORCH_HOME 环境变量或系统默认位置
    torch_home = env.get("TORCH_HOME") or str(Path.home() / ".cache" / "torch")
    ...
    cmd = [
        ...
        str(script_path),
        f"--outdir={output_path}",  # 使用绝对路径
        "--trunc=0.7",
        f"--network={network_path}",  # 使用绝对路径
        f"--test_data={audio_path}",  # 使用绝对路径
        f"--test_img={test_img}",  # 使用绝对路径
        "--motion_guide_img_folder=frames",
        f"--cache_dir={cache_dir}"  # 添加缓存目录参数（绝对路径）
    ]
    ...
    try:
        ...
        # 检查输出目录中的视频文件
        expected_video = Path(output_path) / "output_NeRFFaceSpeech.mp4"
        add_log(f"[NeRF] 检查生成的视频文件: {expected_video}", "info")
        add_log(f"[NeRF] 文件是否存在: {expected_video.exists()}", "info")
        ...
        if process.returncode != 0:
            ...
            # 即使返回码不为0，也检查视频文件是否存在（可能ffmpeg报错但文件已生成）
            if expected_video.exists():
                add_log(f"[NeRF] ⚠️ 虽然返回码不为0，但视频文件已生成: {expected_video}", "warning")
                add_log(f"[NeRF] 文件大小: {expected_video.stat().st_size} 字节", "info")
                return True
            else:
                add_log(f"[NeRF] ❌ 视频文件未生成: {expected_video}", "error")
                # 列出输出目录中的所有文件
                if Path(output_path).exists():
                    files = list(Path(output_path).iterdir())
                    add_log(f"[NeRF] 输出目录中的文件: {[f.name for f in files]}", "info")
                return False
        
        # 返回码为0，检查视频文件是否存在
        if expected_video.exists():
            video_size = expected_video.stat().st_size
            add_log(f"[NeRF] ✅ 视频文件生成成功: {expected_video} (大小: {video_size} 字节)", "success")
            return True
        else:
            add_log(f"[NeRF] ⚠️ 返回码为0，但视频文件不存在: {expected_video}", "warning")
            # 列出输出目录中的所有文件
            if Path(output_path).exists():
                files = list(Path(output_path).iterdir())
                add_log(f"[NeRF] 输出目录中的文件: {[f.name for f in files]}", "info")
