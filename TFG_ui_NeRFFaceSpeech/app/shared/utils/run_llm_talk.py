...
# 尝试导入 TTS 客户端（如果可用）
try:
    from shared.utils.tts_client import (
        check_tts_service_health,
        talk_with_audio_via_service,
        generate_audio_via_service
    )
...
def generate_audio(text: str, output_path: str, character: str) -> Tuple[bool, Optional[str]]:
    """
    生成音频文件，并返回LLM回答文本
    优先使用 TTS 服务（HTTP），如果不可用则回退到 subprocess 方式
    ...
    if TTS_SERVICE_AVAILABLE:
        try:
            if check_tts_service_health():
                add_log("[LLM+TTS] 使用 TTS 服务（HTTP）", "info")
                success, llm_answer = talk_with_audio_via_service(
                    user_input=text,
                ...
                if success:
                    return success, llm_answer
                else:
                    add_log("[LLM+TTS] TTS 服务调用失败，回退到 subprocess 方式", "warning")
            else:
                add_log("[LLM+TTS] TTS 服务不可用，使用 subprocess 方式", "warning")
        except Exception as e:
            add_log(f"[LLM+TTS] TTS 服务调用异常: {e}，回退到 subprocess 方式", "warning")
    
    # 回退到原来的 subprocess 方式
    try:
